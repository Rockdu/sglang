...
sources -> host loader + pools -> bare media ----+
        -> outer source configs ----------------+-> member.process_media
                                                          |
                                                     BatchFeature
                                                          |
...
raw media + IDs -> supported loader -> same IDs + input_text=None, no decode
...
Serving fixtures terminate bare media runs with an ordinary token, matching the
legacy offset scanner contract; its circular-boundary limitation is unchanged.
...
Processing uses HF kwargs merging; cloned workers run off the event loop and are
passed to the member, otherwise processing runs inline like the legacy route.
"""
...
class TestTokenSpaceProcessor(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_supported_loaders_preserve_ids_and_return_bare_media(self):
        ...
        for loader in (processor.load_mm_data, processor.fast_load_mm_data):
            for input_ids, return_text in (
                ...
                with (
                    ...
                    self.assertIsNone(loaded.input_text)
                    image = loaded.images[0]
                    self.assertEqual(image.mode, "RGB")
                    self.assertEqual(image.getpixel((0, 0)), (10, 20, 30))
                    self.assertIs(loaded.videos[0], video_decoder)
    ...
    async def test_fast_loader_ignores_text_only_when_token_space_is_enabled(self):
        processor = self.make_processor()
        input_ids = [10, 99, 11]
        image = Image.new("RGB", (3, 2))
        for enabled in (True, False):
            with self.subTest(use_token_space_processor=enabled):
                processor.use_token_space_processor = enabled
                ...
                self.assertEqual(loaded.input_text, None if enabled else "<image>")
                self.assertIs(loaded.input_ids, input_ids)
    ...
    async def test_loaded_media_is_reusable_without_token_processing(self):
        ...
        self.assertEqual(
            process_images.call_args.kwargs["source_configs"],
            [{"do_resize": False}, {}],
        )
        self.assertEqual(
            process_videos.call_args.kwargs["source_configs"], [{"nframes": 2}]
        )
        self.assertEqual(
            process_audio.call_args.kwargs["source_configs"], [{"sampling_rate": 16000}]
        )
        ...
        with (
            ...
            patch.object(processor, "process_mm_data") as process_mm_data,
        ):
        ...
        forwarded = process_mm_data.call_args.kwargs
    ...
    async def test_disabled_loaders_keep_legacy_text_decoding(self):
        ...
        for prompt, return_text in ((None, True), (input_ids, False)):
            with self.subTest(prompt=prompt, return_text=return_text):
                with self.assertRaises(AssertionError):
                    await processor.fast_load_mm_data(
                        prompt=prompt,
                        multimodal_tokens=None,
                        return_text=return_text,
                    )
    ...
    def test_source_offsets_preserve_empty_items_and_reject_split_segments(self):
        ...
        with self.assertRaisesRegex(ValueError, "token span"):
            processor.sglang_post_process([99, 7, 99, 11], media)
    ...
    def test_serving_postprocessing_preserves_reusable_media_and_full_offsets(self):
        ...
        for input_ids, boundary in (([99, 99], 0), ([99, 99, 99], 2), ([99] * 5, 5)):
            ...
            serving = processor.sglang_post_process(expanded + [11], media)
            self.assertEqual(serving.input_ids, [99] * 5 + [11])
            self.assertEqual(
