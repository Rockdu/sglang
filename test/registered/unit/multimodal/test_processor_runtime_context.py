"""Serving processors keep published settings and share executor lifecycles.
    ...
    startup opt-in -> independent token-space member    disabled -> original route
    legacy construction ignores token-space initialization and keeps its own route
    unsupported model / explicit model opt-out -> original route even with opt-in
    ...
    host worker HF clone -> member media and expansion spec
...
def _check_serving_processor_lifecycle():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        class LegacyOnlyQwenProcessor(QwenVLImageProcessor):
            supports_token_expansion = False

        # Startup opt-in cannot replace an unsupported or explicitly opted-out model.
        for selected_class, model_type in (
            (QwenVLImageProcessor, "intern_s2_preview"),
            (LegacyOnlyQwenProcessor, config.model_type),
        ):
            fallback_config = type(config).from_dict(config.to_dict())
            fallback_config.model_type = model_type
            with patch.dict(
                PROCESSOR_MAPPING,
                {model: selected_class for model in selected_class.models},
                clear=True,
            ):
                fallback = get_mm_processor(
                    fallback_config, worker_args, hf_processor, transport_mode=None
                )
            try:
                assert type(fallback) is selected_class
                assert not fallback.use_token_space_processor
            finally:
                fallback.shutdown()
        ...
        try:
            assert type(processor) is processor_class
            assert (
                type(processor.token_space_process_strategy)
                is QwenTokenSpaceProcessStrategy
            )
            ...
            member = processor.token_space_process_strategy
            member._processor = (
                object()
            )  # Requests must use the host's HF processor clone.
            with (
                patch.object(
                    member, "process_media", wraps=member.process_media
                ) as media,
                patch.object(
                    member,
                    "get_mm_token_expansion_spec",
                    wraps=member.get_mm_token_expansion_spec,
                ) as spec,
            ):
                asyncio.run(process_images(processor, placeholder, image_paths))
            worker_processor = media.call_args.kwargs["processor"]
            assert worker_processor is not hf_processor
            assert spec.call_args.args[0] is worker_processor
            ...
            with (
                ...
                patch.object(
                    ...
                    side_effect=AssertionError(
                        "Legacy construction entered token space"
                    ),
                ),
            ):
