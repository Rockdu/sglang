"""Check multimodal async dispatch and executor-backed preprocessing.
  ...
  startup opt-in AND model supports_token_expansion
    True  -> Base token-space async dispatch (complete media and token boundary)
...
def test_supported_models_delegate_the_complete_request(
    migrated_processor, monkeypatch, media_source
):
    from sglang.srt.multimodal.processors.base_processor import (
        BaseMultimodalProcessor,
    )
    ...
    shared_entry = AsyncMock(return_value=object())
    monkeypatch.setattr(
        BaseMultimodalProcessor, "process_token_space_mm_data_async", shared_entry
    )
    request_kwargs = {
        "image_data": ["image"],
        "video_data": ["video"],
    ...
    assert output is shared_entry.return_value
    shared_entry.assert_awaited_once_with(**request_kwargs)
