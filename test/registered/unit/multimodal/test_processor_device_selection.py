"""Serving resolves the current runtime policy before shared media processing.

    ServerArgs.base_gpu_id + current runtime policy -> serving device
    Explicit trainer call arguments --------------> shared media devices
      Serving device policy --------->|-> member image kwargs <- model video override
                                      |-> member video kwargs <- request / model override
            ...
            with self.subTest(
                ...
                with (
                    ...
                    patch.object(
                        processor,
                        "sglang_post_process",
                        side_effect=lambda input_ids, features, **kwargs: features,
                    ),
                    patch.object(
                        processor,
