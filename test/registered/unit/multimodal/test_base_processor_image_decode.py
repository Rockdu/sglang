...
source -> Base._load_single_item -> decoded image or client/server error
             IO workers         -> original source/rate passed unchanged -> bare media
ImageData / VideoData wrappers -> decoder-owned unwrapping
JPEG + Modality.IMAGE -> shared mixin -> serving / trainer hosts -> same decoder policy
"""
...
class TestLoadSingleItemImageDecode(CustomTestCase):
    ...
    def test_fast_loader_submits_all_media_and_returns_bare_results(self):
        ...
        self.assertEqual(
            loaded.images, ["decoded:first", f"decoded:{sources['image_data'][1]}"]
        )
        self.assertEqual(loaded.videos, ["decoded:video"])
        self.assertEqual(loaded.audios, [f"decoded:{sources['audio_data'][0]}"])
        self.assertCountEqual(
            calls,
            [
                (sources["image_data"][0], Modality.IMAGE, None, 16000, False),
                (sources["image_data"][1], Modality.IMAGE, None, 16000, False),
                (sources["video_data"][0], Modality.VIDEO, None, 16000, False),
                (sources["audio_data"][0], Modality.AUDIO, None, 16000, False),
            ],
        )
